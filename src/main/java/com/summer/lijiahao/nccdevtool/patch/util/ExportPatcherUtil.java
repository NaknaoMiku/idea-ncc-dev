package com.summer.lijiahao.nccdevtool.patch.util;
...
public class ExportPatcherUtil {
    ...
    private final String TYPE_WSDL = ".wsdl";
...
//    private final String TYPE_WSDL = ".wsdl";
    ...
    public void exportPatcher(JProgressBar progressBar) throws Exception {
        ...
        for (String moduleName : modulePathMap.keySet()) {
            ...
            if (outPath == null) {
                throw new Exception("please set output folder or rebuild module ! module:" + moduleName);
            }
    ...
    private void exportProperties(String moduleName, File fromFile) throws Exception {
        ...
        String componentPath = "";
        String fromParentPath = fromFile.getPath();
        if (fromParentPath.contains(PATH_RESOURCES)) {
            if (fromParentPath.split(Matcher.quoteReplacement(PATH_RESOURCES)).length == 2) {
                componentPath = fromParentPath.split(Matcher.quoteReplacement(PATH_RESOURCES))[1];
            }
        }

        outPatcher(moduleName, fromFile.getPath(), toPath + componentPath);
    }
    ...
    private boolean exportJavaOrKotlin(String moduleName, String ncModuleName, String compilerOutputUrl, File fromFile) throws Exception {
        ...
        if (fromFile.lastModified() > new File(compilerOutputUrl + className).lastModified()) {
            throw new Exception(className.substring(1).replace(File.separator, ".") + " is old, please rebuild : " + moduleName);
        }
        //输出补丁
    ...
    private void outPatcher(String moduleName, String srcPath, String toPath) throws Exception {
        ...
        if (!from.exists()) {
            throw new Exception("please build : " + moduleName);
        }
