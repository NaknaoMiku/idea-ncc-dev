package com.summer.lijiahao.debug.util;
...
import com.summer.lijiahao.script.studio.ui.preference.prop.SingleServerInfo;
...
public class CreatApplicationConfigurationUtil {
    ...
    private static void setConfiguration(Module selectModule, ApplicationConfiguration conf, boolean serverFlag) throws BusinessException {
        ...
        if (serverFlag) {
            ...
            //本地调试移动用127.0.0.1
            ipAndPort.setAddress(DEFALUT_IP);
            envs.put("FIELD_CLINET_IP", StringUtils.isBlank(ipAndPort.getAddress()) ? DEFALUT_IP : ipAndPort.getAddress());
            envs.put("FIELD_CLINET_PORT", String.valueOf(ipAndPort.getPort() == null ? DEFALUT_PORT : ipAndPort.getPort()));
        ...
        if (!configurationsList.isEmpty()) {
            for (RunConfiguration configuration : configurationsList) {
                if (configuration instanceof ApplicationConfiguration) {
                    ...
                    setConfiguration(newConf.getConfigurationModule().getModule(), newConf, serverClass.equals(newConf.getMainClassName()));
